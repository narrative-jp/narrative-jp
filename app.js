/* ─────────────────────────────────────────────
   NARRATIVE — Interview Media Site
   Vanilla JS (no frameworks)
   ───────────────────────────────────────────── */

// ── Sample Interview Data (25 entries) ──
const INTERVIEWS = [
  {
    id: "N-001", name: "遠藤 俊介", alias: "遠藤さん（仮名）", age: 25, occupation: "会社員",
    excerpt: "それを遠慮の帰って言うらしいですよ。僕はずっと、自分の言葉で話すことが苦手だった。",
    content: [
      "── まず、今のお仕事について教えてください。",
      "IT企業でエンジニアをしています。新卒で入って3年目ですね。最初はとにかく覚えることが多くて、毎日が必死でした。でも最近になってようやく、自分なりのやり方みたいなものが見えてきた気がします。",
      "[img]images/contents_test_1.JPG",
      "── 学生時代はどんな方でしたか？",
      "正直、目立たないタイプでしたね。友達は少なくなかったけど、自分から何かを発信するのは苦手で。それを遠慮の帰って言うらしいですよ、って先輩に言われたことがあって。そのとき初めて、自分が遠慮してるんだって気づいたんです。",
      "[img]images/contents_test_2.jpeg",
      "── そこから何か変わりましたか？",
      "プログラミングに出会ったのが大きかったです。コードって、自分の考えをそのまま形にできるじゃないですか。言葉にするのが苦手でも、ロジックで表現できる。それが救いだったし、今の仕事にもつながっています。"
    ],
    tags: ["エンジニア", "20代", "東京"]
  },
  {
    id: "N-002", name: "佐藤 美咲", alias: "佐藤さん（仮名）", age: 32, occupation: "フリーランスデザイナー",
    excerpt: "デザインは問題解決だって言うけど、本当は自分自身を解決してたのかもしれない。",
    content: [
      "── デザイナーになったきっかけを教えてください。",
      "大学では文学部だったんです。でも卒業後にたまたまWebデザインのワークショップに参加して、画面の中で自分のアイデアが形になる瞬間に衝撃を受けました。",
      "── フリーランスという選択は？",
      "会社員を5年やって、独立しました。自分のペースで仕事がしたかったというのもあるけど、一番の理由は、もっと多様なプロジェクトに関わりたかったから。今は地方の中小企業のブランディングを中心にやっています。",
      "── デザインとは何ですか？",
      "デザインは問題解決だって言うけど、本当は自分自身を解決してたのかもしれない。自分が何を美しいと思うか、何を大切にしているか。それをクライアントの課題を通して見つけていく作業なんだと思います。"
    ],
    tags: ["デザイナー", "30代", "大阪"]
  },
  {
    id: "N-003", name: "田中 健太", alias: "田中さん（仮名）", age: 28, occupation: "料理人",
    excerpt: "包丁を握ると、世界が静かになるんです。それが僕にとっての瞑想みたいなもの。",
    content: [
      "── 料理人を目指したのはいつ頃ですか？",
      "高校生の頃です。母親が入院して、初めて家族のためにご飯を作ったんです。下手くそだったけど、弟が『おいしい』って言ってくれて。あの瞬間から料理が好きになりました。",
      "── 今はどんなお店で？",
      "都内のフレンチレストランで修行中です。まだまだ下っ端ですけど、毎日新しい発見があって楽しいです。",
      "── 料理の魅力は？",
      "包丁を握ると、世界が静かになるんです。余計なことを考えなくなる。それが僕にとっての瞑想みたいなもの。あと、食べた人の表情が変わる瞬間が好きですね。"
    ],
    tags: ["料理人", "20代", "東京"]
  },
  {
    id: "N-004", name: "山本 真理", alias: "山本さん（仮名）", age: 41, occupation: "小学校教師",
    excerpt: "子どもたちは毎日、大人が忘れたことを思い出させてくれる。",
    content: [
      "── 教師を続けて何年になりますか？",
      "18年目です。最初の頃は本当に大変で、何度も辞めようと思いました。でも続けてこられたのは、やっぱり子どもたちの存在ですね。",
      "── 印象に残っているエピソードは？",
      "不登校だった男の子が、1年かけて少しずつ教室に来られるようになったことです。最後の日に手紙をくれて、『先生がいてくれてよかった』と書いてあった。あの手紙は今でも宝物です。",
      "── 教育で大切にしていることは？",
      "子どもたちは毎日、大人が忘れたことを思い出させてくれる。『なぜ？』って問いかける力とか、失敗を恐れない勇気とか。だから僕も、教えるというより一緒に学ぶ姿勢を大切にしています。"
    ],
    tags: ["教師", "40代", "埼玉"]
  },
  {
    id: "N-005", name: "鈴木 大輔", alias: "鈴木さん（仮名）", age: 35, occupation: "農家",
    excerpt: "土に触れていると、自分がどこから来たのか思い出す気がする。",
    content: [
      "── 農家になった経緯を教えてください。",
      "元々は東京でサラリーマンをしていました。30歳の時に体を壊して、実家の長野に帰ったんです。そこで父の畑を手伝い始めたのがきっかけです。",
      "── 都会との違いは？",
      "全然違いますよ。朝は鳥の声で起きるし、季節の移り変わりを肌で感じる。最初は不便だと思ったけど、今はこの暮らしが当たり前になりました。",
      "── 農業の魅力は？",
      "土に触れていると、自分がどこから来たのか思い出す気がする。種を蒔いて、水をやって、太陽の光を待つ。そのプロセスの中に、人間として大切なことが全部詰まっている気がします。"
    ],
    tags: ["農家", "30代", "長野"]
  },
  {
    id: "N-006", name: "高橋 花", alias: "高橋さん（仮名）", age: 22, occupation: "大学生",
    excerpt: "将来の夢がないことが、ずっとコンプレックスだった。でも今は、それでいいと思える。",
    content: [
      "── 今、大学では何を学んでいますか？",
      "社会学です。人間の行動や社会の仕組みに興味があって選びました。",
      "── 就活はどうですか？",
      "周りはみんな明確な目標を持っているように見えて、焦りますね。将来の夢がないことが、ずっとコンプレックスだった。でも最近、それでいいんじゃないかって思えるようになってきました。",
      "── なぜそう思えるようになったんですか？",
      "ゼミの教授に、『夢がなくても、目の前のことを真剣にやっていれば道は開ける』と言われて。今はとにかく、興味のあることに全力で取り組むようにしています。"
    ],
    tags: ["学生", "20代", "京都"]
  },
  {
    id: "N-007", name: "伊藤 誠一", alias: "伊藤さん（仮名）", age: 58, occupation: "タクシー運転手",
    excerpt: "この街の夜を、誰よりも知っている自信がある。",
    content: [
      "── 運転手歴はどのくらいですか？",
      "30年になります。最初はバスの運転手をしていたんですが、もっと人と話したくてタクシーに転向しました。",
      "── 印象的なお客さんは？",
      "たくさんいますよ。終電を逃したサラリーマン、泣きながら乗ってきた若い女性、深夜に病院に向かう家族。みんなそれぞれの物語を持っている。",
      "── 仕事のやりがいは？",
      "この街の夜を、誰よりも知っている自信がある。裏道も、季節ごとの景色の変化も。お客さんを安全に届けて、『ありがとう』と言われた時が一番嬉しいですね。"
    ],
    tags: ["運転手", "50代", "東京"]
  },
  {
    id: "N-008", name: "渡辺 さくら", alias: "渡辺さん（仮名）", age: 29, occupation: "看護師",
    excerpt: "人の命に関わる仕事だからこそ、自分自身を大切にしないといけない。",
    content: [
      "── 看護師になった理由は？",
      "祖母の介護がきっかけです。専門的な知識があれば、もっと祖母を楽にしてあげられたのにと感じて。",
      "── 大変なことは？",
      "体力的にも精神的にもきついです。夜勤もあるし、患者さんの急変もある。でも人の命に関わる仕事だからこそ、自分自身を大切にしないといけないって学びました。",
      "── 支えになっているものは？",
      "退院する患者さんの笑顔ですね。あと、同僚の存在も大きいです。辛い時はお互いに支え合える仲間がいるから続けられています。"
    ],
    tags: ["看護師", "20代", "神奈川"]
  },
  {
    id: "N-009", name: "中村 翔", alias: "中村さん（仮名）", age: 33, occupation: "ミュージシャン",
    excerpt: "売れない時期が長かったけど、音楽を辞めるという選択肢だけはなかった。",
    content: [
      "── 音楽を始めたのはいつですか？",
      "中学生の時にギターを始めて、高校でバンドを組みました。そこからずっと音楽をやっています。",
      "── プロになるまでの道のりは？",
      "長かったですよ。アルバイトをしながらライブハウスを回る日々が10年くらい続きました。売れない時期が長かったけど、音楽を辞めるという選択肢だけはなかった。",
      "── 今の活動について教えてください。",
      "ソロで活動しています。サブスクで少しずつリスナーが増えてきて、最近やっと音楽だけで生活できるようになりました。小さなライブハウスで、目の前の人に届く音楽を作り続けたいです。"
    ],
    tags: ["音楽家", "30代", "東京"]
  },
  {
    id: "N-010", name: "小林 陽子", alias: "小林さん（仮名）", age: 45, occupation: "書店員",
    excerpt: "本は、誰かの人生を変える力を持っている。私はその橋渡しをしているだけ。",
    content: [
      "── 書店員になったきっかけは？",
      "子どもの頃から本の虫でした。大学を出て一度は出版社に入ったんですが、もっと本と読者に近い場所にいたくなって、書店に転職しました。",
      "── 書店の現状について。",
      "厳しいですよ。電子書籍の普及もあって、町の書店はどんどん減っています。でもだからこそ、リアルな本屋でしかできない体験を大切にしたい。",
      "── 本の力について。",
      "本は、誰かの人生を変える力を持っている。私はその橋渡しをしているだけ。お客さんに『この本に出会えてよかった』と言われると、この仕事をやっていてよかったと思います。"
    ],
    tags: ["書店員", "40代", "福岡"]
  },
  {
    id: "N-011", name: "加藤 雅人", alias: "加藤さん（仮名）", age: 27, occupation: "消防士",
    excerpt: "怖くないと言ったら嘘になる。でも、誰かを助けたいという気持ちが勝つんです。",
    content: [
      "── 消防士を志したきっかけは？",
      "小学生の時に近所で火事があって、消防士さんが必死に消火活動をしているのを見たんです。あの姿がずっと頭に残っていて。",
      "── 現場での経験は？",
      "怖くないと言ったら嘘になる。でも、誰かを助けたいという気持ちが勝つんです。訓練で培った技術と仲間への信頼があるから、現場に向かえる。",
      "── オフの日は何をしていますか？",
      "体力維持のためにランニングをしています。あと、料理が趣味で。現場ではチームで食事を作ることも多いので、レパートリーを増やしたくて。"
    ],
    tags: ["消防士", "20代", "千葉"]
  },
  {
    id: "N-012", name: "吉田 恵", alias: "吉田さん（仮名）", age: 38, occupation: "翻訳家",
    excerpt: "言葉を訳すのではなく、文化を訳している。そう気づいてから仕事が変わった。",
    content: [
      "── どの言語を扱っていますか？",
      "英語と日本語の間の翻訳が中心です。主に文芸翻訳をしています。",
      "── 翻訳の難しさは？",
      "直訳では伝わらないニュアンスがたくさんあるんです。言葉を訳すのではなく、文化を訳している。そう気づいてから、作品への向き合い方が変わりました。",
      "── 翻訳家を目指す人へ。",
      "たくさん読んで、たくさん書いてください。そして、原文の著者が何を伝えたかったのかを想像する力を養ってほしい。技術よりも、共感力が大切な仕事だと思います。"
    ],
    tags: ["翻訳家", "30代", "東京"]
  },
  {
    id: "N-013", name: "松本 裕太", alias: "松本さん（仮名）", age: 31, occupation: "理学療法士",
    excerpt: "患者さんが自分の足で歩けた瞬間、僕も一緒に泣いてしまうことがある。",
    content: [
      "── 理学療法士になった理由は？",
      "高校時代に部活で大怪我をして、リハビリを受けたんです。そのとき担当してくれた先生がすごく丁寧で、『この人みたいになりたい』と思いました。",
      "── 仕事で大切にしていることは？",
      "患者さん一人ひとりのペースに合わせること。焦らせない、でも諦めさせない。その絶妙なバランスを探るのが腕の見せどころだと思っています。",
      "── やりがいを感じる瞬間は？",
      "患者さんが自分の足で歩けた瞬間、僕も一緒に泣いてしまうことがある。その人の努力を一番近くで見てきたから。"
    ],
    tags: ["医療", "30代", "北海道"]
  },
  {
    id: "N-014", name: "井上 香織", alias: "井上さん（仮名）", age: 26, occupation: "パティシエ",
    excerpt: "ケーキは誰かの特別な日のためにある。だから一つも手を抜けない。",
    content: [
      "── パティシエになったきっかけは？",
      "母の誕生日に初めてケーキを焼いたのが始まりです。小学4年生の時。見た目はひどかったけど、母がすごく喜んでくれて。",
      "── 修行時代について。",
      "専門学校を出てからフランスに2年行きました。言葉も通じない中で、朝4時から夜中まで働く日々。でも、あの経験が今の基盤になっています。",
      "── お菓子作りの哲学は？",
      "ケーキは誰かの特別な日のためにある。だから一つも手を抜けない。素材選びから仕上げまで、全てに心を込めています。"
    ],
    tags: ["パティシエ", "20代", "神戸"]
  },
  {
    id: "N-015", name: "木村 哲也", alias: "木村さん（仮名）", age: 52, occupation: "漁師",
    excerpt: "海は毎日違う顔を見せる。だから30年やっても飽きない。",
    content: [
      "── 漁師は何代目ですか？",
      "3代目です。祖父の代からこの港で漁をしています。小さい頃から船に乗っていたので、自然とこの道に入りました。",
      "── 海の魅力は？",
      "海は毎日違う顔を見せる。だから30年やっても飽きない。怖い時もあるけど、大漁の日の喜びは何にも代えがたい。",
      "── 後継者問題について。",
      "息子は東京で働いているので、正直寂しいです。でも無理には言えない。この仕事の素晴らしさを若い人たちに伝えていくことが、今の僕にできることだと思っています。"
    ],
    tags: ["漁師", "50代", "三重"]
  },
  {
    id: "N-016", name: "林 美穂", alias: "林さん（仮名）", age: 34, occupation: "弁護士",
    excerpt: "法律は冷たいものだと思われがちだけど、本質は人を守るためにある。",
    content: [
      "── 弁護士を目指したきっかけは？",
      "中学時代にいじめを受けた経験があります。そのとき、誰も味方になってくれなかった。自分のような人を守れる存在になりたいと思って法律の道を選びました。",
      "── どんな案件を扱っていますか？",
      "家庭問題が多いです。離婚やDV、子どもの親権に関する案件。感情的になりやすい分野だからこそ、冷静でいることを心がけています。",
      "── 法律について思うことは？",
      "法律は冷たいものだと思われがちだけど、本質は人を守るためにある。依頼者が安心して次の一歩を踏み出せるように、全力でサポートしたいです。"
    ],
    tags: ["弁護士", "30代", "東京"]
  },
  {
    id: "N-017", name: "斎藤 隆", alias: "斎藤さん（仮名）", age: 44, occupation: "大工",
    excerpt: "木は生きている。だから家も生きている。そう思って建てています。",
    content: [
      "── 大工になった経緯は？",
      "父親が大工だったので、小さい頃から現場について行っていました。高校を出てすぐ弟子入りして、もう25年以上になります。",
      "── 仕事のこだわりは？",
      "木は生きている。だから家も生きている。そう思って建てています。一本一本の木の性質を見極めて、最適な場所に使う。機械では分からない、手の感覚を大切にしています。",
      "── これからの大工について。",
      "プレカット（機械加工）が主流になってきていますが、手刻みの技術は絶対に残すべきだと思います。若い人にも伝えていきたいですね。"
    ],
    tags: ["職人", "40代", "新潟"]
  },
  {
    id: "N-018", name: "前田 リサ", alias: "前田さん（仮名）", age: 23, occupation: "動物園飼育員",
    excerpt: "動物たちは言葉を話さない。だからこそ、小さな変化を見逃さないようにしている。",
    content: [
      "── 飼育員になるまでの道のりは？",
      "子どもの頃から動物が大好きで、獣医学部に進みました。でも在学中に飼育員の仕事に惹かれて。動物と毎日一緒にいられるこの仕事を選びました。",
      "── 担当している動物は？",
      "今はレッサーパンダの担当です。見た目は可愛いけど、意外と気性が荒い時もあるんですよ。でもそのギャップがまた魅力的で。",
      "── 仕事で心がけていることは？",
      "動物たちは言葉を話さない。だからこそ、小さな変化を見逃さないようにしている。食欲や行動パターンの変化から、体調を読み取る力が大切です。"
    ],
    tags: ["飼育員", "20代", "愛知"]
  },
  {
    id: "N-019", name: "清水 幸太郎", alias: "清水さん（仮名）", age: 67, occupation: "退職・元教授",
    excerpt: "退職して初めて、肩書きのない自分と向き合うことになった。",
    content: [
      "── 退職後の生活はいかがですか？",
      "40年間、大学で研究と教育に携わってきました。退職して初めて、肩書きのない自分と向き合うことになった。最初は正直、何をしていいか分からなかったです。",
      "── 今はどのように過ごしていますか？",
      "地域の公民館で子どもたちに科学実験を教えています。大学の講義とは全然違って、子どもたちの反応がダイレクトで面白い。",
      "── 人生で大切だと思うことは？",
      "好奇心を失わないこと。年齢は関係ありません。知らないことを知る喜びは、いくつになっても新鮮です。"
    ],
    tags: ["研究者", "60代", "宮城"]
  },
  {
    id: "N-020", name: "山田 あかり", alias: "山田さん（仮名）", age: 30, occupation: "花屋",
    excerpt: "花は枯れるからこそ美しい。その儚さを届ける仕事をしている。",
    content: [
      "── 花屋を始めたきっかけは？",
      "OLをしていた時、仕事帰りに立ち寄った花屋で、一輪のバラに心を救われたんです。その日から花に魅了されて、思い切って転職しました。",
      "── お店のこだわりは？",
      "季節の花だけを扱うようにしています。旬の花には、その時期にしかない力がある。お客さんにも季節を感じてほしいんです。",
      "── 花について。",
      "花は枯れるからこそ美しい。その儚さを届ける仕事をしている。花を贈る人の気持ちと、受け取る人の笑顔をつなぐ。それが私の役割です。"
    ],
    tags: ["花屋", "30代", "鎌倉"]
  },
  {
    id: "N-021", name: "岡田 純", alias: "岡田さん（仮名）", age: 36, occupation: "映像作家",
    excerpt: "カメラを通すと、世界はまるで別の場所みたいに見える。",
    content: [
      "── 映像を始めたきっかけは？",
      "大学時代に友達の結婚式のムービーを作ったのが最初です。それまでカメラなんて触ったことなかったけど、映像で物語を作る面白さにハマりました。",
      "── どんな作品を撮っていますか？",
      "ドキュメンタリーが中心です。普通の人の日常を撮るのが好き。ドラマチックな出来事がなくても、人の営みそのものに美しさがあると思っています。",
      "── 映像の力とは？",
      "カメラを通すと、世界はまるで別の場所みたいに見える。普段見過ごしている景色や表情の中に、宝物みたいな瞬間がたくさんある。それを切り取って誰かに届けたいんです。"
    ],
    tags: ["映像", "30代", "京都"]
  },
  {
    id: "N-022", name: "藤田 智子", alias: "藤田さん（仮名）", age: 48, occupation: "助産師",
    excerpt: "命が生まれる瞬間に立ち会えることは、この上ない幸せです。",
    content: [
      "── 助産師を選んだ理由は？",
      "看護師として働いていた時に、たまたま産科に配属されたんです。初めてお産に立ち会った時、こんなにも力強い瞬間があるのかと衝撃を受けました。",
      "── 助産師として大切にしていることは？",
      "お母さんの不安に寄り添うこと。出産は自然なことだけど、初めての方は特に怖いですよね。安心して産める環境を作ることが私の仕事です。",
      "── これまでのキャリアを振り返って。",
      "命が生まれる瞬間に立ち会えることは、この上ない幸せです。何千回経験しても、赤ちゃんの泣き声を聞くたびに感動します。"
    ],
    tags: ["医療", "40代", "広島"]
  },
  {
    id: "N-023", name: "村上 蓮", alias: "村上さん（仮名）", age: 21, occupation: "大学生・起業家",
    excerpt: "失敗を恐れるより、何もしないことの方が怖い。",
    content: [
      "── 大学生で起業したんですね。",
      "はい、大学2年の時にフードデリバリーのアプリを作りました。プログラミングは独学で。最初のバージョンはバグだらけでしたけど。",
      "── 起業のモチベーションは？",
      "地元の商店街がどんどん寂れていくのを見て、テクノロジーで何かできないかと思ったんです。失敗を恐れるより、何もしないことの方が怖い。",
      "── 将来の展望は？",
      "地方の中小企業のDXを支援する会社にしたいです。大学は休学中ですが、いつか必ず卒業します。親には心配かけていますけどね。"
    ],
    tags: ["起業家", "20代", "福井"]
  },
  {
    id: "N-024", name: "長谷川 節子", alias: "長谷川さん（仮名）", age: 72, occupation: "陶芸家",
    excerpt: "土と対話しながら形を作る。70年生きて、まだ土のことが分からない。",
    content: [
      "── 陶芸を始めたのはいつですか？",
      "40歳からです。子育てが一段落して、何か自分のことをしたいと思った時に出会いました。遅いスタートでしたが、それが良かった。人生経験が作品に反映されるから。",
      "── 作品のスタイルについて。",
      "日常使いの器を中心に作っています。美術品ではなく、誰かの食卓に並ぶもの。使うほどに味が出る、そんな器を目指しています。",
      "── 陶芸とは？",
      "土と対話しながら形を作る。70年生きて、まだ土のことが分からない。でもそれが楽しいんです。一生かけても極められない世界があるということは、幸せなことです。"
    ],
    tags: ["芸術家", "70代", "滋賀"]
  },
  {
    id: "N-025", name: "西田 健二", alias: "西田さん（仮名）", age: 39, occupation: "僧侶",
    excerpt: "お寺は死者のためだけの場所ではない。生きている人のための場所でもある。",
    content: [
      "── 僧侶になった経緯は？",
      "実家がお寺なので、自然と。でも最初は継ぎたくなかったんです。大学で哲学を学んで、一般企業に就職もしました。でも30歳の時に父が倒れて、戻る決心をしました。",
      "── お寺の役割について。",
      "お寺は死者のためだけの場所ではない。生きている人のための場所でもある。最近はヨガ教室や子ども食堂を開いて、地域の居場所づくりをしています。",
      "── 仏教から学んだことは？",
      "執着を手放すこと。でもこれが一番難しい。僕自身も毎日修行中です。完璧な人間になるためではなく、自分の不完全さを受け入れるための修行です。"
    ],
    tags: ["僧侶", "30代", "奈良"]
  }
];

// ── Reference Data ──
const REFERENCES = [
  {
    id: "R-001",
    title: "タイトル（仮）",
    source: "著者 / メディア名",
    excerpt: "このリファレンスがプロジェクトに与えた影響についての短いメモ。",
    url: "https://example.com"
  },
  {
    id: "R-002",
    title: "タイトル（仮）",
    source: "著者 / メディア名",
    excerpt: "このリファレンスがプロジェクトに与えた影響についての短いメモ。",
    url: "https://example.com"
  },
  {
    id: "R-003",
    title: "タイトル（仮）",
    source: "著者 / メディア名",
    excerpt: "このリファレンスがプロジェクトに与えた影響についての短いメモ。",
    url: "https://example.com"
  },
  {
    id: "R-004",
    title: "タイトル（仮）",
    source: "著者 / メディア名",
    excerpt: "このリファレンスがプロジェクトに与えた影響についての短いメモ。",
    url: "https://example.com"
  },
  {
    id: "R-005",
    title: "タイトル（仮）",
    source: "著者 / メディア名",
    excerpt: "このリファレンスがプロジェクトに与えた影響についての短いメモ。",
    url: "https://example.com"
  },
  {
    id: "R-006",
    title: "タイトル（仮）",
    source: "著者 / メディア名",
    excerpt: "このリファレンスがプロジェクトに与えた影響についての短いメモ。",
    url: "https://example.com"
  },
];

// ── Utility: Generate gradient background for placeholder thumbnails ──
function generateGradient(index) {
  const hues = [
    [195, 210], [200, 220], [190, 215], [205, 225], [198, 212],
    [192, 218], [202, 208], [196, 222], [188, 214], [204, 216],
  ];
  const [h1, h2] = hues[index % hues.length];
  return `linear-gradient(180deg, hsl(${h1}, 85%, 60%) 0%, hsl(${h2}, 90%, 55%) 100%)`;
}

// ── Simple Hash Router ──
function navigate(path) {
  window.location.hash = path;
}

function getRoute() {
  return window.location.hash || "#/";
}

// ── SVG Icons ──
const ICONS = {
  chevronLeft: '<svg width="9" height="16" viewBox="0 0 9 16" fill="none"><path d="M8 1L1.5 8L8 15" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
  arrowUp: '<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 13V3M8 3L3 8M8 3L13 8" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>',
  external: '<svg width="14" height="14" viewBox="0 0 12 12" fill="none"><path d="M4.5 2H2.5C2.22386 2 2 2.22386 2 2.5V9.5C2 9.77614 2.22386 10 2.5 10H9.5C9.77614 10 10 9.77614 10 9.5V7.5M7 2H10V5M10 2L5.5 6.5" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
  book: '<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M1.5 2.5C1.5 2.5 3 1.5 5 1.5C7 1.5 7 2.5 7 2.5V12C7 12 6.5 11.5 5 11.5C3.5 11.5 1.5 12 1.5 12V2.5Z" stroke="currentColor" stroke-width="1.1" stroke-linecap="round" stroke-linejoin="round"/><path d="M12.5 2.5C12.5 2.5 11 1.5 9 1.5C7 1.5 7 2.5 7 2.5V12C7 12 7.5 11.5 9 11.5C10.5 11.5 12.5 12 12.5 12V2.5Z" stroke="currentColor" stroke-width="1.1" stroke-linecap="round" stroke-linejoin="round"/></svg>'
};

// ── Emoji Cursor ──
function initEmojiCursor() {
  const W = 48, H = 48;
  const cx = 24, cy = 30;
  const fontSize = 40;
  const angle = -22 * Math.PI / 180;

  const tipDist = fontSize * 0.47;
  const hotX = Math.round(cx - Math.sin(-angle) * tipDist);
  const hotY = Math.round(cy - Math.cos(-angle) * tipDist);

  function drawEmoji(scale) {
    const canvas = document.createElement("canvas");
    canvas.width = Math.round(W * scale);
    canvas.height = Math.round(H * scale);
    const ctx = canvas.getContext("2d");
    ctx.scale(scale, scale);
    ctx.save();
    ctx.translate(cx, cy);
    ctx.rotate(angle);
    ctx.font = `${fontSize}px serif`;
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText("\u{1F446}", 0, 0);
    ctx.restore();
    return canvas.toDataURL("image/png");
  }

  const url1x = drawEmoji(1);
  const url2x = drawEmoji(2);

  const style = document.createElement("style");
  style.id = "emoji-cursor-style";
  style.textContent = `
    *, *::before, *::after {
      cursor: image-set(url("${url1x}") 1x, url("${url2x}") 2x) ${hotX} ${hotY}, auto !important;
    }
  `;
  document.head.appendChild(style);
}

// ── Live Timestamp Footer ──
function formatTimestamp() {
  const now = new Date();
  const offsetMin = -now.getTimezoneOffset();
  const sign = offsetMin >= 0 ? "+" : "-";
  const absMin = Math.abs(offsetMin);
  const offsetH = Math.floor(absMin / 60);
  const offsetM = absMin % 60;
  const tzLabel = offsetM === 0
    ? `UTC${sign}${offsetH}`
    : `UTC${sign}${offsetH}:${String(offsetM).padStart(2, "0")}`;

  const Y  = now.getFullYear();
  const Mo = String(now.getMonth() + 1).padStart(2, "0");
  const D  = String(now.getDate()).padStart(2, "0");
  const h  = String(now.getHours()).padStart(2, "0");
  const mi = String(now.getMinutes()).padStart(2, "0");
  const s  = String(now.getSeconds()).padStart(2, "0");
  const ms = String(now.getMilliseconds()).padStart(3, "0");
  return `${Y}${Mo}${D}_${h}${mi}${s}.${ms}_${tzLabel}`;
}

function initLiveFooter() {
  const el = document.getElementById("footer-timestamp");
  if (!el) return;
  const tick = () => {
    el.textContent = `\u00A9 ${formatTimestamp()}  NARRATIVE.JP`;
    requestAnimationFrame(tick);
  };
  requestAnimationFrame(tick);
}

// ── Sort Button (floating) ──
// #sort-float は styled <select>。タップで直接ネイティブピッカーが開く。
function initScrollToTop() {
  const picker = document.getElementById("sort-float");
  if (!picker) return;

  picker.addEventListener("change", (e) => {
    currentSort = e.target.value;
    const route = getRoute();
    if (!route.startsWith("#/interview/") && route !== "#/about" && route !== "#/contact") {
      renderHomePage();
    }
  });
}

// ── 統合スクロール方向ハンドラ
// scroll-top ボタンと view-switcher を同じ挙動で制御する
function initScrollBehavior() {
  const switcher = document.getElementById("view-switcher");

  let lastScrollY = window.scrollY;
  let ticking = false;

  window.addEventListener("scroll", () => {
    if (ticking) return;
    ticking = true;
    requestAnimationFrame(() => {
      const currentScrollY = window.scrollY;
      const scrollingDown  = currentScrollY > lastScrollY;
      const nearTop        = currentScrollY < 60;
      const nearBottom     = currentScrollY + window.innerHeight >= document.documentElement.scrollHeight - 80;

      // ── View Switcher（ホームページ表示中のみ）──
      if (switcher && switcher.classList.contains("visible")) {
        if (nearTop) {
          switcher.classList.remove("hidden");
        } else {
          switcher.classList.toggle("hidden", scrollingDown || nearBottom);
        }
      }

      // ── Sort Float（view-switcher と全く同じ挙動）──
      const sortFloat = document.getElementById("sort-float");
      if (sortFloat && sortFloat.classList.contains("visible")) {
        if (nearTop) {
          sortFloat.classList.remove("hidden");
        } else {
          sortFloat.classList.toggle("hidden", scrollingDown || nearBottom);
        }
      }

      lastScrollY = currentScrollY;
      ticking = false;
    });
  }, { passive: true });
}

// ── Magnetic Hover Effect ──
// カーソルが近づくとアイコンが少しカーソルに吸い寄せられるエフェクト
function initMagneticHover(el, { radius = 140, strength = 0.35 } = {}) {
  if (!el) return;

  // 現在値と目標値（遅延補間でふわっと動かす）
  let targetX = 0, targetY = 0;
  let currentX = 0, currentY = 0;
  let rafId = null;
  let animating = false;

  const animate = () => {
    // Lerp（線形補間）で滑らかに追従
    currentX += (targetX - currentX) * 0.15;
    currentY += (targetY - currentY) * 0.15;

    el.style.transform = `translate(${currentX.toFixed(2)}px, ${currentY.toFixed(2)}px)`;

    // 目標値にほぼ到達したら停止
    if (Math.abs(targetX - currentX) < 0.1 && Math.abs(targetY - currentY) < 0.1) {
      currentX = targetX;
      currentY = targetY;
      el.style.transform = `translate(${currentX}px, ${currentY}px)`;
      animating = false;
      return;
    }
    rafId = requestAnimationFrame(animate);
  };

  const startAnim = () => {
    if (!animating) {
      animating = true;
      rafId = requestAnimationFrame(animate);
    }
  };

  const onMouseMove = (e) => {
    const rect = el.getBoundingClientRect();
    const cx = rect.left + rect.width / 2;
    const cy = rect.top + rect.height / 2;
    const dx = e.clientX - cx;
    const dy = e.clientY - cy;
    const dist = Math.hypot(dx, dy);

    if (dist < radius) {
      // 近いほど強く引き寄せる（距離0→最大、radius→0）
      const falloff = 1 - dist / radius;
      targetX = dx * strength * falloff;
      targetY = dy * strength * falloff;
      startAnim();
    } else if (targetX !== 0 || targetY !== 0) {
      targetX = 0;
      targetY = 0;
      startAnim();
    }
  };

  const onMouseLeave = () => {
    targetX = 0;
    targetY = 0;
    startAnim();
  };

  window.addEventListener("mousemove", onMouseMove, { passive: true });
  window.addEventListener("mouseleave", onMouseLeave);
}

// ── Render: Interview Card ──
// ── View State (Portrait / Pickup) ──
let currentView = "portrait"; // "portrait" | "pickup"

/**
 * 各インタビューのサムネイル画像パスを返す。
 * 画像は images/{id}-portrait.jpg, images/{id}-pickup.jpg の命名規則。
 * 画像が存在しない場合は renderCard 側で onerror フォールバック。
 */
function getThumbSrc(interview, view) {
  // データ側で明示指定されていればそれを優先
  if (view === "pickup" && interview.pickup) return interview.pickup;
  if (view === "portrait" && interview.portrait) return interview.portrait;
  // デフォルト: 命名規則ベース
  return `images/${interview.id}-${view}.jpg`;
}

function renderCard(interview, index) {
  const card = document.createElement("div");
  card.className = "card animate-in";
  card.style.animationDelay = `${Math.min(index * 30, 300)}ms`;
  card.setAttribute("role", "article");
  card.addEventListener("click", () => navigate(`/interview/${interview.id}`));

  const thumbSrc = getThumbSrc(interview, currentView);
  const gradient = generateGradient(index);

  card.innerHTML = `
    <div class="card-thumb-wrapper">
      <div class="card-thumb" style="background: ${gradient}">
        <img
          class="card-thumb-img"
          src="${thumbSrc}"
          alt="${interview.alias}"
          loading="lazy"
          onerror="this.style.display='none'"
        />
      </div>
      <span class="card-id">${interview.id}</span>
      <span class="card-external">${ICONS.external}</span>
    </div>
    <div class="card-body">
      <p class="card-excerpt">${interview.excerpt}</p>
      <div class="card-meta">
        <span class="card-meta-item">${interview.alias}</span>
        <span class="card-meta-item">${interview.age}歳</span>
        <span class="card-meta-item">${interview.occupation}</span>
      </div>
    </div>
    <div class="card-action">
      <button class="card-action-btn">
        ${ICONS.book}
        Read Interview
      </button>
    </div>
  `;

  const actionBtn = card.querySelector(".card-action-btn");
  actionBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    navigate(`/interview/${interview.id}`);
  });

  return card;
}

// ── Reference Card ──
function renderReferenceCard(ref, index) {
  const card = document.createElement("div");
  card.className = "card animate-in";
  card.style.animationDelay = `${Math.min(index * 30, 300)}ms`;
  card.setAttribute("role", "article");
  card.style.cursor = "pointer";
  card.addEventListener("click", () => window.open(ref.url, "_blank", "noopener"));

  const gradient = generateGradient(index);

  card.innerHTML = `
    <div class="card-thumb-wrapper">
      <div class="card-thumb" style="background: ${gradient}"></div>
      <span class="card-id">${ref.id}</span>
      <span class="card-external">${ICONS.external}</span>
    </div>
    <div class="card-body">
      <p class="card-excerpt">${ref.title}</p>
      <div class="card-meta">
        <span class="card-meta-item">${ref.source}</span>
      </div>
    </div>
    <div class="card-action">
      <button class="card-action-btn">
        ${ICONS.external}
        Open Link
      </button>
    </div>
  `;

  const actionBtn = card.querySelector(".card-action-btn");
  actionBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    window.open(ref.url, "_blank", "noopener");
  });

  return card;
}

// ── Page: Home ──
// ── Sort State ──
let currentSort = ""; // デフォルト：並び替えなし（元の掲載順）

function getSortedInterviews() {
  const list = [...INTERVIEWS];
  if (currentSort === "newest") {
    // ID降順（N-025 → N-001）
    list.sort((a, b) => b.id.localeCompare(a.id));
  } else if (currentSort === "oldest") {
    // ID昇順（N-001 → N-025）
    list.sort((a, b) => a.id.localeCompare(b.id));
  } else if (currentSort === "name") {
    list.sort((a, b) => a.name.localeCompare(b.name, "ja"));
  }
  return list;
}

// ── Hero: 都市名をタイムゾーンから取得 ──
function getCityFromTimezone() {
  const tz = Intl.DateTimeFormat().resolvedOptions().timeZone; // e.g. "Asia/Tokyo"
  const city = tz.split("/").pop().replace(/_/g, " ");
  return city.toUpperCase();
}

// ── Hero: 日付フォーマット "TUESDAY, APRIL 14" ──
function formatHeroDate(now) {
  const day  = now.toLocaleDateString("en-US", { weekday: "long" }).toUpperCase();
  const month = now.toLocaleDateString("en-US", { month: "long" }).toUpperCase();
  const date  = now.getDate();
  return `${day}, ${month} ${date}`;
}

// ── Hero: 時刻フォーマット "23:13:04" ──
function formatHeroTime(now) {
  const h  = String(now.getHours()).padStart(2, "0");
  const m  = String(now.getMinutes()).padStart(2, "0");
  const s  = String(now.getSeconds()).padStart(2, "0");
  return `${h}:${m}:${s}`;
}

let heroTimerID = null;

function renderHomePage() {
  const app = document.getElementById("app");
  app.innerHTML = "";
  if (heroTimerID) { clearInterval(heroTimerID); heroTimerID = null; }

  const sorted = getSortedInterviews();
  const city = getCityFromTimezone();

  // ── Hero Section ──
  const hero = document.createElement("div");
  hero.className = "hero";
  const isDark = document.documentElement.classList.contains("dark");
  hero.innerHTML = `
    <p class="hero-title">NARRATIVE</p>
    <p class="hero-date" id="hero-date"></p>
    <p class="hero-time" id="hero-time"></p>
    <div class="theme-toggle-wrap">
      <button
        class="theme-toggle ${isDark ? "is-on" : ""}"
        id="theme-toggle"
        role="switch"
        aria-checked="${isDark}"
        aria-label="ダークモード切替"
      ><span class="theme-toggle-thumb"></span></button>
    </div>
  `;
  app.appendChild(hero);

  // 毎秒更新
  const updateHero = () => {
    const now = new Date();
    const dateEl = document.getElementById("hero-date");
    const timeEl = document.getElementById("hero-time");
    if (dateEl) dateEl.textContent = formatHeroDate(now);
    if (timeEl) timeEl.textContent = `${city}, ${formatHeroTime(now)}`;
  };
  updateHero();
  heroTimerID = setInterval(updateHero, 1000);

  // Count badge
  const grid = document.createElement("div");
  grid.className = "card-grid";
  sorted.forEach((interview, idx) => {
    grid.appendChild(renderCard(interview, idx));
  });
  app.appendChild(grid);
}

// ── Page: Detail ──
function renderDetailPage(id) {
  const interview = INTERVIEWS.find(i => i.id === id);
  const app = document.getElementById("app");
  app.innerHTML = "";

  if (!interview) {
    app.innerHTML = `
      <div class="detail-container">
        <button class="detail-back" id="detail-back-btn">${ICONS.chevronLeft} Back</button>
        <p>Interview not found.</p>
      </div>
    `;
    document.getElementById("detail-back-btn").addEventListener("click", () => navigate("/"));
    return;
  }

  const index = INTERVIEWS.indexOf(interview);

  const container = document.createElement("div");
  container.className = "detail-container animate-in";

  const contentHtml = interview.content.map(p => {
    if (p.startsWith("[img]")) {
      const src = p.replace("[img]", "");
      return `<img class="detail-content-img" src="${src}" alt="" loading="lazy" />`;
    }
    return `<p class="${p.startsWith("──") ? "question" : ""}">${p}</p>`;
  }).join("");

  const tagsHtml = interview.tags.map(tag =>
    `<span class="detail-tag">${tag}</span>`
  ).join("");

  container.innerHTML = `
    <button class="detail-back" id="detail-back-btn">${ICONS.chevronLeft} NARRATIVE</button>

    <div class="detail-hero">
      <div class="detail-hero-inner" style="background: ${generateGradient(index)}">
        <img
          class="detail-hero-img"
          src="${getThumbSrc(interview, 'portrait')}"
          alt="${interview.alias}"
          onerror="this.style.display='none'"
        />
      </div>
    </div>

    <div class="detail-id">${interview.id}</div>
    <h1 class="detail-headline">${interview.excerpt}</h1>

    <div class="detail-profile">
      <div class="detail-profile-avatar" style="background: ${generateGradient(index)}">
        ${interview.name.charAt(0)}
      </div>
      <div class="detail-profile-info">
        <span class="detail-profile-name">${interview.alias}</span>
        <span class="detail-profile-meta">${interview.age}歳 / ${interview.occupation}</span>
      </div>
    </div>

    <div class="detail-tags">${tagsHtml}</div>

    <div class="detail-content">${contentHtml}</div>
  `;

  app.appendChild(container);

  document.getElementById("detail-back-btn").addEventListener("click", () => navigate("/"));
  window.scrollTo(0, 0);
}

// ── Page: Shop ──
function renderShopPage() {
  const app = document.getElementById("app");
  app.innerHTML = "";
  app.classList.add("shop-page");

  const container = document.createElement("div");
  container.className = "shop-soon animate-in";
  container.innerHTML = `<span>Soon🏗️</span>`;
  app.appendChild(container);


}

// ── Page: About ──
function renderAboutPage() {
  const app = document.getElementById("app");
  app.innerHTML = "";

  const occupationCount = new Set(INTERVIEWS.map(i => i.occupation)).size;
  const minAge = Math.min(...INTERVIEWS.map(i => i.age));
  const maxAge = Math.max(...INTERVIEWS.map(i => i.age));

  const container = document.createElement("div");
  container.className = "about-container animate-in";

  container.innerHTML = `
    <div class="about-body">
      <h2 class="about-section-title">WHAT IS NARRATIVE?</h2>
      <p>
        電車で隣に座っている人も、<br class="br-sp"/>いつも行くコンビニの店員も、<br class="br-pc"/><br class="br-sp"/>運転席で黙っているタクシー運転手も。
      </p>
      <p>
      　私たちにはそれぞれの生い立ちがあり、<br class="br-sp"/>今日を生きています。
      </p>
      <p>
        子どもの頃になにを見て、昨日なにを考えて、<br class="br-pc"/><br class="br-sp"/>明日なにをしようとしているのか。<br class="br-pc"/><br class="br-sp"/>インタビューを通して「誰か」である誰かの<br class="br-sp"/>人生を記録するメディアです。
      </p>

      <div class="about-section-divider">📮</div>
      <h2 class="about-section-title">CONTACT</h2>
      <p>
        NARRATIVEではインタビューを<br class="br-pc"/><br class="br-sp"/>受けて下さる方を募集しています。<br class="br-sp"/>自薦・他薦は問いません。<br class="br-pc"/><br class="br-sp"/>こちらまでお問い合わせください。
      </p>
      <a href="mailto:hello@narrative.jp" style="color: var(--color-accent); text-decoration: underline; font-size: 18px;">
        hello@narrative.jp
      </a>

      <div class="about-section-divider">🤹‍♂️</div>
      <h2 class="about-section-title">INSPO</h2>
      <p>
        本プロジェクトを始めるきっかけになった<br class="br-pc"/><br class="br-sp"/>作品・メディア・書籍・授業などです。<br class="br-pc"/><br class="br-sp"/>これらは語ることの意味、<br class="br-sp"/>記録することの価値、編集することの楽しさを<br class="br-pc"/><br class="br-sp"/>教えてくれたものたちです。
      </p>
    </div>
  `;

  container.style.paddingBottom = "0";
  app.appendChild(container);

  // Reference card-grid — about-container の外に置いてホームと同じスタイルを保持
  const refGrid = document.createElement("div");
  refGrid.className = "card-grid";
  refGrid.style.paddingTop = "var(--spacing-md)";
  REFERENCES.forEach((ref, idx) => {
    refGrid.appendChild(renderReferenceCard(ref, idx));
  });
  app.appendChild(refGrid);
  window.scrollTo(0, 0);
}

// ── Router ──
function handleRoute() {
  if (heroTimerID) { clearInterval(heroTimerID); heroTimerID = null; }

  // フッターをデフォルトで表示に戻す
  const footer = document.querySelector(".footer");
  if (footer) footer.style.display = "";

  // shop-page クラスをリセット
  document.getElementById("app").classList.remove("shop-page");

  const route = getRoute();
  const isHome = !route.startsWith("#/interview/") && route !== "#/about" && route !== "#/contact" && route !== "#/shop";

  if (route.startsWith("#/interview/")) {
    const id = route.replace("#/interview/", "");
    renderDetailPage(id);
  } else if (route === "#/about") {
    renderAboutPage();
  } else if (route === "#/shop") {
    renderShopPage();
  } else {
    renderHomePage();
  }

  // View Switcher の表示制御（ホームのみ表示）
  const viewSwitcher = document.getElementById("view-switcher");
  if (viewSwitcher) {
    viewSwitcher.classList.toggle("visible", isHome);
  }

  // Sort Float の表示制御（view-switcher と同じ: ホームのみ表示）
  const sortFloat = document.getElementById("sort-float");
  if (sortFloat) {
    sortFloat.classList.toggle("visible", isHome);
    if (!isHome) sortFloat.classList.remove("hidden");
  }

  // Navbar メニューのステータス表示
  const navbarMenu = document.getElementById("navbar-menu");
  if (navbarMenu) {
    if (route === "#/about") {
      navbarMenu.value = "about";
    } else if (route === "#/shop") {
      navbarMenu.value = "shop";
    } else {
      navbarMenu.selectedIndex = 0; // "NARRATIVE"
    }
  }
}

// ── Init ──
document.addEventListener("DOMContentLoaded", () => {
  // Navbar events
  const navbarMenu = document.getElementById("navbar-menu");
  if (navbarMenu) {
    navbarMenu.addEventListener("change", (e) => {
      const value = e.target.value;
      if (value === "home") {
        navigate("/");
        window.scrollTo(0, 0);
      } else if (value === "about") {
        navigate("/about");
      } else if (value === "contact") {
        window.location.href = "mailto:hello@narrative.jp";
      } else if (value === "shop") {
        navigate("/shop");
      }
    });
  }

  // Navbar count label
  const navbarCountLabel = document.getElementById("navbar-count-label");
  if (navbarCountLabel) {
    navbarCountLabel.textContent = `${INTERVIEWS.length} Interviews`;
  }

  // 中央アイコン：ダークモード/ライトモード切り替え
  // navbar-icon: ダークモード機能を解除（トップページへ戻す）
  document.getElementById("navbar-icon").addEventListener("click", () => {
    navigate("/");
    window.scrollTo(0, 0);
  });

  // Theme Toggle（ヒーロー内トグル）- delegateでDOMが入れ替わっても動作
  document.getElementById("app").addEventListener("click", (e) => {
    const btn = e.target.closest("#theme-toggle");
    if (!btn) return;
    const html = document.documentElement;
    const isDark = html.classList.toggle("dark");
    btn.classList.toggle("is-on", isDark);
    btn.setAttribute("aria-checked", isDark);
    try { localStorage.setItem("theme", isDark ? "dark" : "light"); } catch (_) {}
  });

  // View Switcher (Portrait / Pickup)
  const viewSwitcher = document.getElementById("view-switcher");
  if (viewSwitcher) {
    viewSwitcher.querySelectorAll(".view-switcher-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        const view = btn.dataset.view;
        if (view === currentView) return;
        currentView = view;

        // ボタンのアクティブ状態更新
        viewSwitcher.querySelectorAll(".view-switcher-btn").forEach((b) => {
          const active = b.dataset.view === view;
          b.classList.toggle("is-active", active);
          b.setAttribute("aria-selected", active ? "true" : "false");
        });
        // インジケーター位置をdata属性で制御
        viewSwitcher.dataset.view = view;

        // ホーム画面表示中なら再描画
        const route = getRoute();
        if (!route.startsWith("#/interview/") && route !== "#/about" && route !== "#/contact") {
          renderHomePage();
        }
      });
    });
  }

  // Init features
  initEmojiCursor();
  initLiveFooter();
  initScrollToTop();
  initScrollBehavior();
  // マウスポインターを持つデバイス（非タッチ）のみ magnetic hover を有効化
  if (window.matchMedia("(pointer: fine)").matches) {
    initMagneticHover(document.getElementById("navbar-icon"), { radius: 140, strength: 0.35 });
  }

  // Route
  handleRoute();
  window.addEventListener("hashchange", handleRoute);
});
